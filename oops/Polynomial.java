...
public class Polynomial {
	...
	public Polynomial add(Polynomial p){ 
		...
		int plen2 = p.degCoeff.length; int len = Math.min(plen1, plen2); 
		int i; 
		for(i = 0; i < len; i++) { 
    ...
    public Polynomial multiply(Polynomial p){ 
		...
		for(int i = 0; i < this.degCoeff.length; i++){
			...
			int termdeg = i + j; int termCoeff = this.degCoeff[i] * p.degCoeff[j]; int oldCoeff = ans.getCoeff(termdeg); ans.setCoefficient(termdeg, termCoeff + oldCoeff); 
			} 
		} 
		return ans; 
    }		
